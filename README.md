...
### 1. Час виконання запитів

- **До рефакторингу**: Час виконання запитів коливається від 4.973 мс до 226.318 мс.
- **Після рефакторингу**: Час виконання запитів значно зменшився, з діапазоном від 3.026 мс до 105.249 мс. Це свідчить про покращення продуктивності після рефакторингу.

### 2. Використання пам'яті

- **До рефакторингу**: Використання пам'яті для запитів варіюється від 47 МБ до понад 60 МБ. Наприклад, запит "Submit Application" використовує 63,279,104 байтів.
- **Після рефакторингу**: Використання пам'яті значно зменшено. Наприклад, для запиту "SELECT dormitories" для різних `dormitory_id` пам'ять коливається від 260,848 байт до 745,20 байт після рефакторингу, що є значно менше, ніж в попередніх запитах (понад 62 МБ).

### 3. Час виконання при повторних запитах

- **До рефакторингу**: Час виконання повторних запитів (наприклад, "Fetch Dormitories" повторно) не має значних змін, і варіюється в межах 4.973 мс.
- **Після рефакторингу**: Час виконання повторних запитів зменшився або залишився на тому ж рівні, що також свідчить про ефективнішу обробку запитів.

## Висновок

- Після рефакторингу спостерігається значне покращення продуктивності, особливо в часі виконання запитів. Це може бути результатом оптимізації запитів або покращеного управління пам'яттю.
- Значне зниження використаної пам'яті вказує на те, що обсяг даних, що зберігаються в пам'яті під час обробки запитів, був зменшений.
- Загалом, рефакторинг покращив швидкодію та ефективність використання системних ресурсів.


# Аналіз коду з використанням ESLint, JSHint та DeepSource

Для забезпечення високої якості коду в проєкті були використані три інструменти для статичного аналізу: **ESLint**, **JSHint** та **DeepSource**. Ось чому було обрано саме їх:

## 1. ESLint
**ESLint** є одним з найпопулярніших інструментів для аналізу JavaScript коду. Він забезпечує:

- **Гнучкість і налаштування**: ESLint дозволяє налаштовувати правила кодування під специфічні вимоги проєкту, використовуючи популярні стилі (наприклад, Airbnb або Standard).
- **Розширення для IDE**: ESLint підтримує плагіни для редакторів коду (наприклад, Visual Studio Code), що дозволяє здійснювати перевірку на льоту.
- **Пошук помилок**: Виявлення потенційних помилок, неузгодженостей і нетипових стилістичних рішень на етапі написання коду.

## 2. JSHint
**JSHint** — це ще один популярний лінтер для JavaScript, який забезпечує:

- **Легкість у використанні**: Легко налаштовується і використовується для перевірки базових помилок в коді.
- **Ретроактивна підтримка**: Ідеально підходить для старих проєктів, де не всі сучасні інструменти можуть бути сумісними.
- **Швидкість роботи**: Швидко аналізує код, що дозволяє інтегрувати його в швидкий цикл розробки.

## 3. DeepSource
**DeepSource** використовує потужний набір правил для виявлення не лише синтаксичних помилок, але й логічних помилок, проблем з продуктивністю, безпекою та іншими аспектами:

- **Глибокий аналіз коду**: Виявляє складні помилки, проблеми з продуктивністю та безпекою на ранніх етапах.
- **Інтеграція з CI/CD**: Можна інтегрувати з процесом CI/CD для автоматичної перевірки коду при кожному коміті.
- **Покращення безпеки та продуктивності**: Допомагає виявляти небезпечні конструкції та неефективні алгоритми.

## Чому ці інструменти?
Використання ESLint, JSHint та DeepSource дозволяє комплексно покрити різні аспекти аналізу коду. **ESLint** і **JSHint** забезпечують перевірку на помилки та підтримку стилю коду, в той час як **DeepSource** дозволяє знаходити глибші проблеми, такі як логічні помилки, проблеми з продуктивністю та безпекою.
